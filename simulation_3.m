...
%     for obs_iter = 1:n_obstacles
    ...
    epsilon = 0.1;
    for j = 1:length(eta0)
        if safe_mode(j) == 0 && Bar_F(i, j)*mu <= tau - epsilon
            jump_to_safe = 1;
        elseif safe_mode(j) > 0 && Bar_F(i, j) >= tau*mu
            jump_to_invariance = 1;
        end
    end
    if jump_to_safe
        safe_mode = safe_mode + (ones(n_obstacles, 1) - safe_mode) .* (dist <= tau - epsilon);
    elseif jump_to_invariance
        safe_mode = safe_mode .* (dist <= tau);
    end
    for j = 1:length(eta0)
        current_obs = j;
...
function [c1, c2, div] = pick_qp_params(radius_obs, radius)
    c1 = 80  + 350 * radius_obs + min(50 * radius, 100); 
